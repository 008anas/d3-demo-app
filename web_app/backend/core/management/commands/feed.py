...
class Command(BaseCommand):
    ...
    def feed_genetic_elements(self):
        ...
        for e in json_data:
            count += 1
            try:
                GeneticElement(id=e.get('id'), role=e.get('role'), name=e.get('name'), glyph_thumbnail=e.get('glyph_thumbnail'),
    ...
    def feed_matrices(self):
        count = 0
        json_data = self.open_json_file(os.path.join(self.path, '../../data/matrices.json'))
        for m in json_data:
            count += 1
            try:
                Matrix(id=m.get('id'), name=m.get('name'), alias=m.get('alias'), specie_id=m.get('specie'),
                       genetic_element_id=m.get('genetic_element'),
                       matrix_file=m.get('matrix_file'), active=m.get('active'),
        ...
        self.stdout.write(
            self.style.SUCCESS('Matrix table was feeded successfully. ' + str(count) + ' records added.'))
